...
export default function Cart(){
      const [addtocartData,setaddtocartData]=useState<productType[]>([])
...
function getLocalStorageData(){
      const getData = localStorage.getItem("addtocart")
       if(getData){
            ...
            const addItemData = parseData.map((item:productType[])=>(
                {
                    ...item ,
                    quantity: 1
                }
    ...
    function handleQunantityChange(index :number ,operator: "increment"| "decrement"){
        setaddtocartData((preData:productType[])=>{
        const updateData = preData.map((item:productType,i:number)=>{
          if(i === index){
...
useEffect(()=>{
    calculateTotal(addtocartData)
        ...
        function calculateTotal(item:productType[]){
...
// } 
    ...
    if (loading) return <div className="mt-44 flex justify-center ">Loading.....</div>;
    ...
    return(
        ...
        <div className="grid md:grid-cols-3 gap-4">
            <div className="md:col-span-2 bg-gray-100 p-4 rounded-md">
                <h2 className="text-2xl font-bold text-textcolor">Your Carts</h2>
                <hr className="border-gray-300 mt-4 mb-8" />

                <div className="space-y-4">
                    {
                        addtocartData && addtocartData.length > 0 ? (
                        addtocartData.map((item:productType,i:number)=>( 
                    <div 
                    key={i}
                 ...
                 className="w-full object-cover"
                            ...
                            <div>
                                <h3 className="text-base font-bold text-textcolor">{item.title}</h3>
                                    ...
                                    <div>
                                        <button type="button"
                                         ...
                                         className="w-2.5 fill-current" viewBox="0 0 124 124">
                                            ...
                                            <span className="mx-2.5 text-textcolor">{item.quantity}</span>
                                            <svg xmlns="http://www.w3.org/2000/svg"
                        ...
                        <div className="ml-auto">
                            <h4 className="text-base font-bold text-textcolor">${item.quantity*item.price}</h4>
                        </div>
                    </div>
    ...
    <p>cart is empty</p>
            ...
            <div className="bg-gray-100 rounded-md p-4 md:sticky top-0">
                ...
                <ul className="text-textcolor mt-8 space-y-4">
                ...
                <div className="mt-8 space-y-2">
                    ...
                    // }}
                     type="button" className="text-sm px-4 py-2.5 w-full font-semibold tracking-wide bg-textcolor hover:shadow-lg text-white rounded-md">Checkout</button>
                    <button 
                    // onClick={()=>router.push("/product")}
                    type="button" className="text-sm px-4 py-2.5 w-full font-semibold tracking-wide bg-backgroundcolor hover:bg-textcolor hover:text-stone-300 text-textcolor rounded-md">Continue Shopping  </button>
                </div>
            </div>
        </div>
    </div>
